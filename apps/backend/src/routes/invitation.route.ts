...
import { AcceptInvitationResponse, InvitationTokenSchema } from '../schemas/invitation.schema.ts';
...
const app = createInternalApp<{ Variables: AuthVariables }>().basePath('/invitation');
...
app.openapi(acceptInvitationRoute, async (c) => {
  ...
  const addPermissionPromise = addPermissionForPlan(userId, invitation.planId, invitation.hasWritePermission);
