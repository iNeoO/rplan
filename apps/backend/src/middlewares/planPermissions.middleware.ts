...
export const hasReadPlanPermissionsMiddleware = planPermissionsFactory.createMiddleware(async (c, next) => {
...
export const hasWritePlanPermissionsMiddleware = planPermissionsFactory.createMiddleware(async (c, next) => {
  const userId = c.get('userId');
  const planId = c.req.param('id');
